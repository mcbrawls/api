...
import net.mcbrawls.api.database.StatisticUtils
import net.mcbrawls.api.db.DatabaseFactory
import net.mcbrawls.api.database.schema.ApiKeys
import net.mcbrawls.api.database.schema.ChatLogs
import net.mcbrawls.api.database.type.ChatResult
import net.mcbrawls.api.database.type.DbChatMode
...
import net.mcbrawls.api.database.response.MessageCountResponse
import net.mcbrawls.api.database.response.PartnershipResponse
import net.mcbrawls.api.database.response.Profile
import net.mcbrawls.api.database.response.Rank
import net.mcbrawls.api.database.response.Session
...
internal class ApiServer(url: String, credentials: Credentials, schema: String, permissionsSchema: String) {
    private val database = DatabaseFactory.create("jdbc:mysql://$url/$schema", credentials, ::configureDb)
    private val permissionsDatabase = DatabaseFactory.create("jdbc:mysql://$url/$permissionsSchema", credentials, ::configureDb)

    fun configureDb(config: HikariConfig) {
        config.driverClassName = "com.mysql.cj.jdbc.Driver"
        config.maximumPoolSize = 16
        config.connectionTimeout = 60 * 1000
    }

    fun start(port: Int, auth: Boolean) {
            ...
            install(Authentication) {
                ...
                basic("auth-basic") {
                    ...
                    validate { credentials ->
                        ...
                        if (!auth) return@validate UserIdPrincipal(id)
